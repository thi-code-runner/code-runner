...
func before() {
	...
	service := NewService(context.Background(), &cs, s)
	codeRunner = service
...
func TestCRSavesContainerInMap(t *testing.T) {
	...
	err := codeRunner.Execute(ctx, "test-id", ExecuteParams{Writer: wsWriter})
	if err != nil {
		t.Error(err)
	}
	if _, ok := codeRunner.containers["mock-container-id"]; !ok {
		t.Fail()
	}
}
func TestCRSavesReservedContainerInMap(t *testing.T) {
	...
	if _, ok := codeRunner.reservedContainers["test-image"]; !ok {
		t.Fail()
	}
}
...
func TestCRNoCmdID(t *testing.T) {
	...
	var wsBuf bytes.Buffer
	wsWriter := wswriter.WSWriter{Con: &wsBuf, Type: wswriter.WriteOutput}
	err := codeRunner.Execute(ctx, "nonexistent-cmdID", ExecuteParams{Writer: &wsWriter})
	if err == nil {
		t.Error(err)
	}
...
func TestCRNoContainerID(t *testing.T) {
	...
	var wsBuf bytes.Buffer
	wsWriter := wswriter.WSWriter{Con: &wsBuf, Type: wswriter.WriteOutput}
	err := codeRunner.Execute(ctx, "test-id-no-copy", ExecuteParams{Writer: &wsWriter})
	if err == nil {
		t.Error(err)
	}
